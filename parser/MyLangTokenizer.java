...
public class MyLangTokenizer implements Tokenizer {
	...
	static {
		 ...
		 * remark: groups must correspond to the ordinal of the corresponding enum Group
		 * constant
		 */

		/*
		 * builds the regular expression for symbols
		 * symbols must be in reversed String order because the regular expression
		 * operator '|' is left-preferential!
		...
		for (var s : symbols.keySet())
			symbolList.addFirst("\\" + String.join("\\", s.split(""))); // every char is pre-pended with "\\" to avoid
																		// regular expression syntax problems
		final var symbolRegEx = "(" + String.join("|", symbolList) + ")"; // group 1: symbols
		/*
		 * builds the regular expressions for the other groups
		 * remark: keywordRegEx uses word boundary '\b' since keywords match only if the
		 * next symbol is not a letter
		 */
